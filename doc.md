...
```
time docker build -t XXX BUILD_ARG --build-arg APP_NAME=$(echo XXX | perl -pe 's/[a-zA-Z]+(?:-[0-9]+){1,}-//g') --build-arg OS_VERSION=$(echo XXX | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . UNKO
```

- キャッシュ有効-フォアグラウンド実行

```
time docker build -t XXX BUILD_ARG --build-arg APP_NAME=$(echo XXX | perl -pe 's/[a-zA-Z]+(?:-[0-9]+){1,}-//g') --build-arg OS_VERSION=$(echo XXX | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```

- キャッシュ無効

```
time docker build --no-cache -t XXX BUILD_ARG --build-arg APP_NAME=$(echo XXX | perl -pe 's/[a-zA-Z]+(?:-[0-9]+){1,}-//g') --build-arg OS_VERSION=$(echo XXX | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
