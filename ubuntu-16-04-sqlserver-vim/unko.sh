...
while read SRC_FILE_PATH;do
  ...
  while read MY_GROUP_ID MY_GROUP_NAME MY_USER_ID MY_USER_NAME MY_USER_PWD;do
    ...
    while read TEMPLATE_FILE_PATH;do
      ...
      #ubuntuかつroot
      if [ 0 -eq $? ];then
        echo "sed 's/MY_GROUP_ID/$MY_GROUP_ID/;s/MY_GROUP_NAME/$MY_GROUP_NAME/;s/MY_USER_ID/$MY_USER_ID/;s/MY_USER_NAME/$MY_USER_NAME/;s/MY_USER_PWD/$MY_USER_PWD/;' $TEMPLATE_FILE_PATH >$TGT_FILE_PATH"
      else
        echo
      fi
