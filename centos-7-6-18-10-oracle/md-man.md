...
# dockerコンテナ作成
```
docker run --privileged --shm-size=8gb --name centos-7-6-18-10-oracle -itd -v /run/udev:/run/udev -v /run/systemd:/run/systemd -v /tmp/.X11-unix:/tmp/.X11-unix -v /var/lib/dbus:/var/lib/dbus -v /var/run/dbus:/var/run/dbus -v /etc/machine-id:/etc/machine-id -p 28787:8787 -p 21521:1521 -p 25500:5500 centos-7-6-18-10-oracle /sbin/init
```

# dockerコンテナ潜入
```
docker exec --user root -it centos-7-6-18-10-oracle /bin/bash
```
