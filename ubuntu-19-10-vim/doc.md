# dockerイメージ作成
```
time docker build -t ubuntu-19-10-vim --build-arg NNN= --build-arg CONTAINER_NAME=ubuntu-19-10-vim --build-arg OS_VERSION=$(echo ubuntu-19-10-vim | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
