...
```
time docker build --no-cache -t centos-7-6-18-10-ibm-db2 BUILD_ARG --build-arg REPO=script-repo --build-arg OS_VERSION=$(echo centos-7-6-18-10-ibm-db2 | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . UNKO
```

- キャッシュ有効-フォアグラウンド実行

```
time docker build -t centos-7-6-18-10-ibm-db2 BUILD_ARG --build-arg REPO=script-repo --build-arg OS_VERSION=$(echo centos-7-6-18-10-ibm-db2 | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```

- キャッシュ無効

```
time docker build --no-cache -t centos-7-6-18-10-ibm-db2 BUILD_ARG --build-arg REPO=script-repo --build-arg OS_VERSION=$(echo centos-7-6-18-10-ibm-db2 | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
