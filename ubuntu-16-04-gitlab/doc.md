# dockerイメージ作成
```
time docker build -t ubuntu-16-04-gitlab --build-arg NNN= --build-arg CONTAINER_NAME=ubuntu-16-04-gitlab --build-arg OS_VERSION=$(echo ubuntu-16-04-gitlab | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
