...
>$HOME/$ENV_REPO/env-expose-list.md
...
while read tgt;do
  ...
  | sed -r 's/ /|/g;s/^/|/;s/$/|/'
done < <(find $HOME/$ENV_REPO -mindepth 1 -type d | grep -vP '\.git|docker-log') | sort | sed '1i|環境ディレクトリ名|外部公開ポート番号|内部使用ポート番号|' | sed '2i|:--|:-:|:-:|' >>$HOME/$ENV_REPO/env-expose-list.md
