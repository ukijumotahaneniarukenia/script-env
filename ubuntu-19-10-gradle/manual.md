...
```
docker run --privileged --shm-size=12gb --name ubuntu-19-10-gradle -itd -v /run/udev:/run/udev -v /run/systemd:/run/systemd -v /tmp/.X11-unix:/tmp/.X11-unix -v /var/lib/dbus:/var/lib/dbus -v /var/run/dbus:/var/run/dbus -v /etc/machine-id:/etc/machine-id -v /etc/localtime:/etc/localtime:ro ubuntu_gradle-doma
```

# dockerコンテナ潜入

```
docker exec -it ubuntu-19-10-gradle /bin/bash
```
