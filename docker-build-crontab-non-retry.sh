...
pre-process(){
  exec 1>$HOME/script-env/docker-build-log/docker-build-$(date +\%Y-\%m-\%d_\%H-\%M-\%S).stdout.non-retry.log
  exec 2>$HOME/script-env/docker-build-log/docker-build-$(date +\%Y-\%m-\%d_\%H-\%M-\%S).stderr.non-retry.log
}
...
non-retry(){
  ...
  for((RETRY_ROUND_CNT=1;RETRY_ROUND_CNT<=$RETRY_MX_CNT;RETRY_ROUND_CNT++));do
    ...
    bash $HOME/script-env/docker-build-parallel-non-retry.sh $(printf '%02g' $RETRY_ROUND_CNT) &
