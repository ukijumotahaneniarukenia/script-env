...
登録がされているか確認する際は、環境個別を意識していると思う。

```
/usr/local/lib
```

という文言を記載したファイルを登録しておく。

環境個別に追加するのはめんどいが、システム単位の設定が効いているどうか確認するためにも入れといたほうが、便利じゃないかとおもった。

script-envレポに関しては上記のような対応をとって、tobeはシステム単位のみの記載に統一したい。
