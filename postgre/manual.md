...
```
yum install -y --nogpgcheck https://s3.amazonaws.com/rstudio-ide-build/server/centos6/x86_64/rstudio-server-rhel-1.2.1568-x86_64.rpm
```
...
#dockerコンテナ起動
```
docker run --privileged -v /etc/localtime:/etc/localtime  --name postgresql -p 87:8787 -p 5432:5432 -itd centos_postgresql /sbin/init
```

#dockerコンテナ潜入
```
docker exec --user postgres -it postgresql /bin/bash
```
