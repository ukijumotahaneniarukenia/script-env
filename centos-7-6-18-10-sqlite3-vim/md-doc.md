...
```
time docker build -t centos-7-6-18-10-sqlite3-vim --build-arg GIT_VERSION=2-24-1 --build-arg PYTHON_VERSION=3-7-4 --build-arg SQLITE3_VERSION=3-30-0 --build-arg REPO=script-repo --build-arg APP_NAME= --build-arg OS_VERSION=$(echo centos-7-6-18-10-sqlite3-vim | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . UNKO
```
