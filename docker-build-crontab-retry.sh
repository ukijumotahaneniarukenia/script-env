...
logger(){
  ...
  local TGT_BUILD_IMAGE_NON_DONE_CNT=$(echo "$TGT_BUILD_IMAGE_EXPECT" | grep -vP "$(echo "${TGT_BUILD_IMAGE_ACTUAL:-UNKO}"|xargs|tr ' ' '|')"|wc -l)
  ...
  echo "$TGT_BUILD_IMAGE_EXPECT" | grep -vP "$(echo "${TGT_BUILD_IMAGE_ACTUAL:-UNKO}"|xargs|tr ' ' '|')"
}
