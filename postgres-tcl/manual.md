...
# dockerイメージ作成
```
time docker build -t centos_postgres-tcl . | tee log
```
...
# dockerコンテナ起動
```
docker run --privileged --shm-size=8gb --name postgres-tcl -itd -v /etc/localtime:/etc/localtime -v /run/udev:/run/udev -v /run/systemd:/run/systemd -v /tmp/.X11-unix:/tmp/.X11-unix -v /var/lib/dbus:/var/lib/dbus -v /var/run/dbus:/var/run/dbus -v /etc/machine-id:/etc/machine-id centos_postgres-tcl
```

# dockerコンテナ潜入
```
docker exec --user postgres -it postgres-tcl /bin/bash
```
