...
>$HOME/$ENV_REPO/app-env-build-arg-list.md
...
while read tgt;do
  ...
  | sed -r 's/ /|/g;s/^/|/;s/$/|/'
done < <(find $HOME/$ENV_REPO -mindepth 1 -type d | grep -vP '\.git|docker-log') | sort | sed '1i|環境ディレクトリ名|key|value|' | sed '2i|:--|:--|:--|' >>$HOME/$ENV_REPO/app-env-build-arg-list.md
