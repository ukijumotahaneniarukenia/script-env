...
```
time docker build -t centos-7-6-18-10-x --build-arg PYTHON_VERSION=3-7-4 --build-arg GIT_VERSION=2-24-1 --build-arg CONTAINER_NAME=centos-7-6-18-10-x --build-arg OS_VERSION=$(echo centos-7-6-18-10-x | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . 1>~/script-env/centos-7-6-18-10-x/log 2>&1 &
```
...
```
time docker build --no-cache -t centos-7-6-18-10-x --build-arg PYTHON_VERSION=3-7-4 --build-arg GIT_VERSION=2-24-1 --build-arg CONTAINER_NAME=centos-7-6-18-10-x --build-arg OS_VERSION=$(echo centos-7-6-18-10-x | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
