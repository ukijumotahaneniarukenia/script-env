# dockerイメージ作成
```
time docker build -t centos-7-6-18-10-x --build-arg NNN= --build-arg CONTAINER_NAME=centos-7-6-18-10-x --build-arg OS_VERSION=$(echo centos-7-6-18-10-x | grep -Po '[a-z]{1,}(?:-[0-9]{1,}){1,}') . | tee log
```
