...
while read tgt;do
  ...
  #デフォルト設定を適用
  [ -z "$RT" ] && printf "sed -i 's;EXPOSE;%s;' %s\n" "$(echo "grep EXPOSE $HOME/script-env/$tgt/md-env.md" | bash 2>/dev/null | sed 's;.*=;;' | sort | uniq)" $HOME/script-env/$tgt/md-doc.md | bash
  #環境個別の設定を適用
  [ -z "$RT" ] || printf "sed -i 's;EXPOSE;%s;' %s\n" "$RT" $HOME/script-env/$tgt/md-doc.md | bash

  RT="$(echo "grep '' $HOME/script-env/$tgt/env-shm-size.md" | bash 2>/dev/null)"
  #デフォルト設定を適用
  [ -z "$RT" ] && printf "sed -i 's;SHM_SIZE;%s;' %s\n" "$(echo "grep bashM_SIZE $HOME/script-env/md-env.md" | bash | sed 's;.*=;;' | sort | uniq)" $HOME/script-env/$tgt/md-doc.md | bash
  #環境個別の設定を適用
  [ -z "$RT" ] || printf "sed -i 's;SHM_SIZE;%s;' %s\n" "$(echo $RT | sed 's/SHM_SIZE=//g')" $HOME/script-env/$tgt/md-doc.md | bash
...
while read tgt;do
  ...
  if [ "XXX" == "XXX$RESULT" ] ;then
    ...
    while read tgt;do
      echo "sed -i 's;YYY;;g' $HOME/script-env/$tgt/md-doc.md" | bash
    done
  else
    #デフォルトユーザー以外を使用
    ...
    while read tgt;do
      ...
      echo "sed -i 's;YYY;$NNN;g' $HOME/script-env/$tgt/md-doc.md" | bash
    done
  fi
done
