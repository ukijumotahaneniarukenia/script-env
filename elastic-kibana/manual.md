...
# dockerイメージ作成
```
time docker build -t centos_elastic-kibana . | tee log
```

# dockerコンテナ起動
```
docker run --privileged -itd --name elastic-kibana -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 5601:5601 -p 9200:9200 centos_elastic-kibana
```

# dockerコンテナ潜入
```
docker exec -it elastic-kibana /bin/bash
```
