  ...
  if [ -s $tgt/$INPUT_FILE_NAME ];then
    {
        ...
        cat $tgt/$INPUT_FILE_NAME | perl -pe 's/.*=//g'
    } | xargs
  else
    ...
    echo "[$(echo $tgt | perl -pe 's;.*/;;g')]($BASE_URL/$ENV_REPO/blob/master/$(echo $tgt | perl -pe 's;.*/;;g')/$INPUT_FILE_NAME)" $DOCKERFILE_IMAGE_VERSION
  fi \
  | perl -pe 's/(?<=md\)) /|/;s/^/|/;s/$/|/'
